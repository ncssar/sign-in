...
# TMG 3-23-19 modification of https://github.com/Skucul/timepicker
...
#             self.selected = self.min
    ...
    def _genitems(self, *a):
        self.clear_widgets()
#         print("\n_genitems called; color="+str(self.color)+"\n")
        for i in range(*self.range):
            if i % self.multiples_of != 0:
                continue
            n = Number(text=self.number_format_string.format(i), size_factor=self.number_size_factor, color=self.color+[1])
...
class CircularTimePicker(BoxLayout):
    ...
    def _get_time(self):
        if self.time_list[1]==60:
            self.time_list[1]=0
#         print("_get_time called: time_list="+str(self.time_list))
